...
int MCP3008::readADC(byte adcnum)
{
  // algo
 ...
  byte commandout = adcnum & 0x07;  // Reduce to a valid ADC
  commandout |= 0x18;               // start bit + single-ended bit
  commandout <<= 3;                 // we only need to send 5 bits here
  
  for (int i=0; i<5; i++)
  {
    digitalWrite(_mosipin, (commandout & 0x80)?HIGH:LOW );
    commandout <<= 1;
 ...
    digitalWrite(_clockpin, LOW);
    adcout <<= 1;
    if (digitalRead(_misopin)) adcout |= 0x01;
  }
  digitalWrite(_cspin, HIGH);
